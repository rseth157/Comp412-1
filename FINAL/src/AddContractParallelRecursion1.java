...
public class AddContractParallelRecursion1 {

	int[][] graph;
	public List<Integer> polynomial;
	static int i = 0;
	private final Object lock = new Object();
	public AddContractParallelRecursion1(int[][] graph) {
		this.graph = graph;
	...
	public void chromaticPolynomial(int[][] G, int th){
		...
		Chromatic c = new Chromatic(G, th);
		pool.invoke(c);
	}
	...
	public static void main(String args[]) throws InterruptedException {
		...
		AddContractParallelRecursion1 ga = new AddContractParallelRecursion1(g);
	...
	private static class Count implements Runnable {
		AddContractParallelRecursion1 ga;
		int j;
		BigInteger[] coeff;
		BigInteger one = new BigInteger("1");

		public Count(AddContractParallelRecursion1 ga, int j, BigInteger[] coeff) {
			this.ga = ga;
			this.j = j;
			this.coeff = coeff;
		}
	...
	public class Chromatic extends RecursiveTask<Integer> {
		...
		public Chromatic(int[][] G, int noOfThread) {
		...
		public Integer compute() {
			...
			if (isGraphNotCompleteAddEdge(G, k)) {
				...
				int[][] G1 = G;
				// contract graph
				int[][] G2 = contractGraph(G1, k[0], k[1],noOfThread);
				// chromaticPolynomial(G1);
				// chromaticPolynomial(G2);

				Chromatic left = new Chromatic(G1,noOfThread);

				left.fork();

				Chromatic right = new Chromatic(G2,noOfThread);
	...
	public class Contract implements Runnable {
		int REMOVE_COLUMN;
		int REMOVE_ROW;
		int[][] contracted;
		int vertex2;
		...
		private Contract(int REMOVE_COLUMN, int REMOVE_ROW, int[][] contracted,
				int vertex2, int[][] g, int p, int i) {
	...
	private int[][] contractGraph(int[][] g, int vertex, int vertex2,
		...
		if (g.length == 1) {
			...
			for (int i = 0; i < g.length; ++i) {
				...
				executor.execute(new Contract(REMOVE_COLUMN, REMOVE_ROW,
						contracted, vertex2, g, p, i));
